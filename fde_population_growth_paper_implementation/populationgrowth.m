...
initial_guesses_fractional = [1.2, 0.003; 0.06, 0.005; 1, 0.007];
...
xlim_range = [1927.04275 1927.0429];
ylim_range = [2016.398745 2016.39876];
...
for i = 1:length(initial_guesses_classical)
    fprintf('Classical (P0=%.3f) -> Error: %f, Optimized P: %.6f\n', ...
        initial_guesses_classical(i), errors_classical(i), optimized_P_classical(i));
end

for j = 1:size(initial_guesses_fractional, 1)
    fprintf('Fractional (α0=%.1f, P0=%.3f) -> Error: %f, Optimized α: %.3f, Optimized P: %.6f\n', ...
    ...
    % Calculate efficiency gain only if corresponding classical model exists
